...
16. Sends `Diff` the whole desired `orders` table plus an index, three times. Without `files` it is refused: `legacy` has `R__order_totals` recorded, and a diff that cannot see the migration directory cannot tell the view it declares from an object nothing manages. With the `R__` pair in `files` the view is built on the desired schema too, so the response is only the up SQL from what `legacy` has now to it (`ADD COLUMN`, `CREATE INDEX CONCURRENTLY`), the down SQL back, and `repeatableObjects: ["public.order_totals"]`. The third call, with the schema `legacy` already has, reports no changes.
17. Stops a run at a version: two migrations are submitted together with `toVersion` on the first, and only that one is applied — the plan still carries both, with the second marked `withheld`, so the report cannot be read as the whole set. Asking for the same version again is refused: it now selects nothing while the migration above it is still pending, and `--to` never reverts. A plain run lands the rest.
18. Registers a `tenant` target with `--search-path tenant,public` and runs a migration creating an unqualified table called `migrations`: it lands in `tenant`, while `godwit.migrations` next to it is still the journal with that migration recorded in it.
19. Lists every run through the CLI (`godwit runs`) from inside a replica — the same binary, the same API; the `KIND` column tells baseline runs from migrations.
20. Scrapes `/metrics` on the surviving replica: runs per state, the reconciler takeover, the refused hazard and the drift check all show up as Prometheus series.
