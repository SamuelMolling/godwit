...
`godwit lint` parses it offline; `godwit plan --target` says what the twelve generated statements come to:

```
godwit will perform the following actions:

  # 20260904180100_total_cents  (written by a directive)
  -- godwit: change-type orders.total bigint using='(total * 100)::bigint'
  ~ table "public"."orders" {
      ~ total     = numeric -> bigint
      + total_old = numeric NULL
        # (3 unchanged attributes hidden)
    }
  note: leaves public.orders.total_old for rollback; drop it with `-- godwit: drop-column public.orders.total_old`
```

`total_old` is the point: the old values are still there until you drop them. `--plan-format statements` prints the twelve statements themselves, with the `-- godwit expanded:` marker above each block the expander generated — everything under a marker up to the next one was written by godwit rather than by hand:
...
```
  ...
  statement 3, runs in batches over id (int), 5000 rows per transaction
      WITH b AS (SELECT id AS godwit_key FROM public.orders WHERE id > $1::bigint ...);
  statement 8, runs inside a transaction, contract phase
