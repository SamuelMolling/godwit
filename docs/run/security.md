...
**What a target's registration gives back.** `GetTarget` (`godwit target show`, `operator`) returns everything in that config column except the credential: the Vault path, the mounted file, the template, and for `static` only `dsn_registered: true`. The sealed DSN is never returned and never opened to be redacted — a read path that decrypts is a read path that can leak — and nothing needs it back, because `RegisterTarget` keeps it while another setting changes. **That is a property of a type, not of a handler.** `controlplane.TargetRegistration` is the registration with no field a credential could live in, and the RPC, the audit entry and the server log are all rendered from one: nothing that is shown is built from the config map directly, so the credential has no path onto any of them. Adding a field to that struct is therefore the moment to check this page — it is the one place the guarantee could be given away. The template is returned in full and is therefore held to being a template: one carrying a literal password instead of `{{password}}` is refused at registration, so that column cannot become a place a secret hides. `operator` rather than `read` is the same line drawn once more: `ListTargets` says how a target behaves, `GetTarget` says where its secret lives, and a Vault address (`read`, above) plus a path is a map.
...
A job running `command: apply`, `confirm`, `revert` or a real `migrate` holds a `pipeline` token, which by the threat model above is the target's own credential. The Action's guards decide who may make that job run, so they authorise rather than advise. Full mechanics in [who may command an apply](../use/github-actions.md#who-may-command-an-apply).
...
**The apply needs an approval, and GitHub is the one that says whether there is one.** With `require-approval` (default `true`), `apply` and `confirm` read `GET /pulls/{n}/reviews`, take the latest review of each reviewer, and need one of them to be `APPROVED`. godwit does not rule on whether an approval has gone stale: *Dismiss stale pull request approvals when new commits are pushed* is a branch protection setting, and with it on GitHub marks the review `DISMISSED` itself, which this check already refuses. A repository that leaves it off has decided a push does not withdraw an approval, and godwit is not the place to overrule that. The review that *triggers* an `apply-on: approve` job is still checked against its own `commit_id`, because that is the command racing a push, not the approval going stale.
...
- **An approval is not a statement about a commit unless the repository makes it one.** Without stale dismissal on the branch protection rule, an approval given on an earlier commit still counts — to GitHub's merge button and therefore to godwit. Teams that need "apply only what was reviewed" turn that setting on; it is one switch, it covers the merge too, and it is theirs.
- **`require-approval: "false"` removes the review requirement entirely.** `godwit apply <sha>` in the comment body is then the only anchor left, and is refused when the head has moved past it.
...
**The App's private key is read from a file or from the environment, never from an argument.** It is the App's whole identity — it mints an installation token for every repository the App is installed on — so there is no flag carrying the PEM: `--github-private-key-file` names a path, and `GODWIT_GITHUB_PRIVATE_KEY` carries the key itself. A flag would put it in `/proc/<pid>/cmdline`, which is world-readable on the node and shows up in any process listing. Between the two that remain, the file is stronger: it keeps the key out of the environment, which a sidecar, a core dump and `kubectl exec -- env` all read. The environment form is what most deployments will use, because the Secret the pod already sources holds it, and that is the same exposure the store DSN and the master key have. Setting both fails `serve`, so which one is in use is never a question of precedence.
