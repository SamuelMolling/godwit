...
| `--github-private-key-file` | `GODWIT_GITHUB_PRIVATE_KEY_FILE` | PEM file holding the App's private key (PKCS#1 or PKCS#8). `GODWIT_GITHUB_PRIVATE_KEY` carries the PEM itself instead; setting both fails `serve`. There is deliberately no flag for the PEM itself, so it cannot be read out of `/proc/<pid>/cmdline` |
...
`--github-webhook-addr` set without a webhook secret, without an app id, or with a private key that is not PEM, fails `serve` before the store is opened, as does an unknown value in `--github-allowed-associations`. So does naming the private key twice: `GODWIT_GITHUB_PRIVATE_KEY` alongside `--github-private-key-file` is refused rather than resolved by precedence.
...
| `GODWIT_PUBLIC_URL` | no | base URL of the UI as a reader reaches it. The service uses it for the "Open run" button in Slack messages; the CLI uses it to link the run and its plan from [`godwit run report`](../use/cli.md#godwit-run-report), so set it in the workflow too when the Action posts the apply report. Both compose `<url>/ui/runs/<id>` |
...
| `GODWIT_GITHUB_PRIVATE_KEY` | with `--github-webhook-addr`, unless the key comes from a file | the App's private key as PEM. One of this and `--github-private-key-file` is required; both together fail `serve` |
...
The flags each command takes and the token scope it needs. What a command is *for*, what it prints and what it exits with is the [command reference](../use/cli.md).

Global: `--config <path>`. Every service command: `--server`, `--token`, `--json`.
...
| `godwit up` | `--dsn` (required), `--dir`, `--lock-timeout`, `--statement-timeout` | runs the executor directly against a database; on a database with no history a checkpoint in the directory runs and the versions it collapses are recorded ([checkpoints](../internals/journal.md#checkpoints)). Local pair with `down`, as `migrate`/`revert` are the service pair |
...
Lint codes: `E001` directory failed to load, `E002` parse error, `E003` migration modified after merge (needs `--base`), `E004` malformed or misplaced `-- godwit:` directive, including an `assert` whose query is not a single read-only `SELECT` of one column ([directives](../internals/admission.md#directives), [assertions](../internals/admission.md#assertions)), `E005` the migration generated from the declared `schema_source` is out of date ([schema sources](../internals/drift.md#schema-sources)), `H001`–`H010` unacknowledged hazards on the up side, `W001` no-op down migration and `W002` schema source not checked because no server was given (warnings, never blocking). Hazard findings carry a `recipe` (the safe SQL, [hazards](../internals/admission.md#hazards)): indented under the finding in text, a `<details>` block per finding in markdown, a field in JSON.
...
| `godwit target show <name>` | | operator; the provider, where the credential is read from, and every registered setting. A `static` target prints `dsn registered true` and never the DSN |
...
| `godwit checkpoint` | `--name <snake_case>` (required), `--dir` (read and written to), `--at <version>`, `--dry-run` | read; collapses the versioned migrations at or below `--at` (the newest by default) into `<timestamp>_<name>.up.sql`, a file carrying the schema they produce and no down side ([checkpoints](../internals/journal.md#checkpoints)). The service replays them on a scratch database and refuses unless the generated body reproduces the same schema fingerprint; `--dry-run` prints it without writing |
...
| `godwit run report <run-id>` | `--command <name>` (the report heading; defaults to the run's kind), `--plan-format schema\|statements`, `--format text\|markdown` | read; reads the run and the plan it was bound to. `GODWIT_PUBLIC_URL` is what makes the run and plan links resolve |
...
`migrate --plan <id>` binds that plan explicitly: target, rollout and files come from the plan unless `--target`, `--rollout` or `--dir` are given (then they must agree with it); it cannot be combined with `--dry-run` or with `--to`. Files are sent as `<version>_<name>.up.sql` / `.down.sql` or `R__<name>.up.sql` / `.down.sql` bodies.
