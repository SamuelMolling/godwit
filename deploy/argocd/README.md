...
The PreSync Job passes no plan id. The service computes the plan key from the migration bodies, the target and the rollout, and binds the run to the `ready` plan with that key — the one `godwit plan --target orders --save` (or the Action with `command: plan`, which passes `--save` for you) stored on the pull request. When the pull request already applied (the Action's default `apply-on-pr` mode: `/godwit apply` before the merge), the same key finds the `bound` plan and the Job re-attaches to its `succeeded` run, exit 0 without touching the target: the hook is then a verification, and the migrations of the sync are already in place before any pod changes. Teams that want the hook to be the only thing that applies keep the Action in `mode: apply-on-merge` (or `plan` only) on the pull request. The ConfigMap below carries the same file contents as the repository, so the key matches; keep `--target` and `--rollout` on the Job identical to the pull request job. If the target moved since the plan (a hand change, an unexplained history entry) the Job exits 3 with the `PlanStale` report; re-plan on the pull request or `godwit drift accept orders` when the change is intended.
