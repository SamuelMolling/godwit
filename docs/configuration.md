...
| `godwit diff` | `--target`, `--schema <file>` (required), `--name <snake_case>` (required unless `--dry-run`), `--dir`, `--dry-run` | read; writes `<timestamp>_<name>.up.sql` / `.down.sql` from the live target to the schema file; prints the up SQL with hazards and recipes, `no changes` when they match, `--dry-run` prints without writing |
