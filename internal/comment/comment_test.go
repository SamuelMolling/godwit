...
func TestParseFound(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"among prose", "ship it\n\ngodwit apply\nthanks", "apply", comment.Command{Name: "apply"}},
		{"after a fenced block", "```\ngodwit apply --nope\n```\ngodwit apply", "apply", comment.Command{Name: "apply"}},
		{"after a tilde block", "~~~\ngodwit apply --nope\n~~~\ngodwit apply", "apply", comment.Command{Name: "apply"}},
		...
		{"first line wins", "godwit revert\ngodwit apply", "", comment.Command{Name: "revert"}},
		{"want skips another command", "godwit revert --force\ngodwit apply", "apply", comment.Command{Name: "apply"}},
	} {
...
func TestParseSilence(t *testing.T) {
	...
	for _, tc := range []struct{ name, body, want string }{
		...
		{"not a whole line", "please godwit apply", "apply"},
		{"inline code", "`godwit apply`", "apply"},
		{"fenced", "```\ngodwit apply\n```", "apply"},
		{"tilde fenced", "~~~\ngodwit apply\n~~~", "apply"},
		{"unclosed fence", "```\ngodwit apply", "apply"},
