...
The App's private key reaches the process one of two ways, and `existingSecret.githubPrivateKey` picks which. Named — the default, `github-private-key.pem` — that Secret entry is projected as a file at `serve.githubApp.privateKeyPath` and read with `--github-private-key-file`; the chart mounts nothing else, and the PEM is not in the environment, which a sidecar, a core dump and `kubectl exec -- env` all read. Empty, no volume is mounted and no file flag rendered: the PEM comes through `envFrom` as `GODWIT_GITHUB_PRIVATE_KEY`, which is what a Secret written for godwit's environment already holds. The file is the stronger of the two and the reason the default names an entry; the environment is not a hole, it is the same Secret reaching the process the same way its DSNs do. The webhook secret and the app id are single-line and are always environment; the app id is not a secret at all.
