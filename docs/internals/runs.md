...
A command carried by the GitHub App holds no token, and its actor is `github:<owner>/<repo>:<login>`: the repository the binding authorised and the login GitHub attested, which is the login whose write access was checked before anything ran. Either half alone answers only part of "who was allowed to do this", so both are in the field, repository first. A plan nobody commanded reads `github:<owner>/<repo>:(autoplan)` — a push produced it, and the head sitting in the bound repository is the whole of its authorisation ([decision 0016](../decisions/0016-the-app-is-bound-to-targets-by-the-server.md#amendment--the-commander-is-part-of-the-actor)). Parentheses cannot appear in a GitHub login, so a marker can never be read as a person.
