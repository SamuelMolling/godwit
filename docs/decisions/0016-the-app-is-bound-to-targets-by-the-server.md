...
3. **An approving review.** `GET /repos/{owner}/{repo}/pulls/{n}/reviews`, same grouping by reviewer, and the approver's own permission is checked too. *Withdrawn in #129, following [0007's amendment](0007-the-action-authorises-with-permission-and-approval.md#amendment--the-platform-says-whether-a-pull-request-is-approved): the `commit_id` anchor and the author comparison are gone from this path as well, and godwit takes GitHub's answer to "is it approved". The grouping stays, so a later `CHANGES_REQUESTED` still supersedes.*
4. **A command older than the head is refused.** `godwit apply <sha>` and the `commit_id` of a review whose body carried the command are compared against the head resolved live, at the moment of the command. These are guards on the command, not on the approval, and 0007's amendment leaves them standing.
