...
| Revert, baseline, status | `RevertRun` runs the down side of every file the run carried through the same gate — the whole directory, not the last version ([runbook](docs/runbook.md#reverting-a-run)); `BaselineTarget` adopts an existing database; `GetTargetStatus` answers applied/pending/last run/drift in one call. |
