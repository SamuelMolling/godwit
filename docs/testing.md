...
`Validate` creates a scratch database, replays the history migration by migration through the same `Executor` a target uses, then applies the pending plans and snapshots the schema after each one. Those milliseconds are round trips, and **fourteen of them were `bootstrap`** re-creating tables that already existed: `Executor.apply` ensured the journal schema on every plan, and the replay builds one `Executor` per migration over one connection.

`engine.Session` now carries that fact on the connection, so bootstrap runs once per database. Counted at the `DB` interface, a replayed migration costs **19 round trips instead of 33** (33 and 19 include the six the bounded advisory lock and the session setup take since the chaos findings landed, and 33 the transaction bootstrap now runs in): three migrations through one session cost 33, 19, 19, and three through an `Executor` each still cost 33, 33, 33.

**A 1000-migration replay, timed directly** — one connection, nothing else on the daemon, four rounds alternating which side goes first:

| | Before | After |
|---|---:|---:|
| round trips per migration | 30 | **19** |
| 1000 migrations replayed | 5.03–5.79 s | **3.55–3.75 s** |
| per migration | 5.4 ms | **3.7 ms** |

**About 31% off the replay** — close to the 37% of the round trips that went away, which says the replay is latency-bound rather than CPU-bound. The saving therefore scales with the round trip: eleven of them are 1.7 ms a migration on a local socket, and eleven times the RTT against a database a network hop away.

It does **not** follow that `plan` gets 31% faster. The replay is about an eighth of a `plan` at this size, so end to end this is worth a couple of seconds in forty. `make load` was run before and after, twice each alternating, and its `plan` came out at 96.5 s / 69.7 s before and 51.1 s / 37.5 s after — but the daemon's load fell monotonically across those four runs (each series drops by the same 27% between its own two), so that spread is the machine, not the change. The direct measurement above is the one that means something.

What the remaining 19 are:

| Round trips | What |
|---:|---|
| 6 | the advisory lock and the session it is taken on: `current_database()`, `application_name` and keepalives, and the bounded `pg_advisory_lock` in its own transaction, then the unlock |
| 5 | the work: the statement, its `done` journal row, the run row, the `migrations` row, the run closed |
| 4 | `BEGIN`/`COMMIT` around the statement, and again around `finalize` |
| 2 | `SET LOCAL lock_timeout` and `SET LOCAL statement_timeout`, per statement |
| 2 | the two reads before the run: is this version applied, and is there a run to resume |

`pgx.Batch` would collapse the statement's five into one and `finalize`'s four into one — both are fixed sequences inside one transaction already, so the journal's guarantee is untouched — at the cost of the per-statement `Duration` the observer reports. On a scratch database the six lock round trips protect nothing: the replay owns a database it just created and drops. Neither is done here.
...
**1000 migrations was also where the default request limit stopped.** Every `plan`, `migrate`, `verify` and `diff` sends the whole directory, two files per migration, and `--max-files` defaulted to 2000: at exactly 1000 the directory fit, the 1001st did not, and neither did the one extra file `godwit checkpoint` writes. Admission is now counted in migrations — `--max-migrations` 2000, `--max-files` 5000 — so this directory and its checkpoint clear the defaults; the rig still raises both when `GODWIT_LOAD_HISTORY` is set past them.
...
| A directory of 1000 migrations is the ceiling on the default `--max-files 2000`: every request carries the whole directory at two files per migration, and a checkpoint adds a file rather than removing any — the collapsed migrations stay until every target is above the checkpoint and someone deletes them by hand. | `internal/limits/limits.go` | fixed: admission counts migrations (`--max-migrations` 2000) and the file cap became a bound on everything else (`--max-files` 5000); `TestCheckListing` pins a 1000-migration directory and its checkpoint as admissible |
| Two in five of the scratch replay's round trips are `bootstrap` re-creating tables that exist, once per migration. | `internal/engine/executor.go` | fixed: `engine.Session` carries the bootstrap on the connection, so it runs once per database and a migration costs 19 round trips instead of 33 — 31% off the replay, a couple of seconds of a forty-second `plan`. `bootstrap` also stopped depending on `IF NOT EXISTS` being race-free: it runs in one transaction under an advisory lock |
