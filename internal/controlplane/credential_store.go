...
func (s *Store) RegisterCredentialStore(ctx context.Context, cs CredentialStore) error {
	if _, err := s.pool.Exec(ctx,
		`INSERT INTO cp_credential_stores (name, address, k8s_role, k8s_mount, k8s_jwt, token_env)
		 VALUES ($1, $2, $3, $4, $5, $6)
		 ON CONFLICT (name) DO UPDATE SET address = EXCLUDED.address, k8s_role = EXCLUDED.k8s_role,
		   k8s_mount = EXCLUDED.k8s_mount, k8s_jwt = EXCLUDED.k8s_jwt, token_env = EXCLUDED.token_env`,
		cs.Name, cs.Address, cs.Role, cs.Mount, cs.JWTPath, cs.TokenEnv); err != nil {
...
func (s *Store) ListCredentialStores(ctx context.Context) ([]CredentialStore, error) {
	rows, err := s.pool.Query(ctx, `
		SELECT s.name, s.address, s.k8s_role, s.k8s_mount, s.k8s_jwt, s.token_env,
	...
	if _, err := pgx.ForEachRow(rows, []any{&cs.Name, &cs.Address, &cs.Role, &cs.Mount, &cs.JWTPath, &cs.TokenEnv, &cs.Targets}, func() error {
		out = append(out, cs)
...
func (s *Store) VaultStore(ctx context.Context, name string) (creds.VaultStore, error) {
	...
	err := s.pool.QueryRow(ctx,
		`SELECT address, k8s_role, k8s_mount, k8s_jwt, token_env FROM cp_credential_stores WHERE name = $1`,
		name).Scan(&v.Address, &v.Role, &v.Mount, &v.JWTPath, &v.TokenEnv)
