...
| UI | `serve --ui` serves an operator UI at `/ui`: needs-you queue, run timeline with resume/park/confirm/revert, the bound plan with its hazard recipes, drift events and accept. Sign in with any bearer token's secret as the basic-auth password (`ui:<token name>`, the token's scope) or with `--ui-user` / `--ui-password` / `--ui-scope`; pages offer only what the scope allows and anything beyond it is a `403`. Actions audited as `ui:<name>`. |
