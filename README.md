...
| Drift detection | Schema fingerprint after each run; a monitor diffs the live schema, records events, notifies and auto-resolves. One open event per target and diff, enforced by a unique index, so replicas checking at the same instant never double-record or double-notify. `AcceptBaseline` blesses manual changes. |
