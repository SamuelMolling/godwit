...
- **A checkpoint's own body is planned and applied by the executor**, with the journal, the same as any migration — which is also how the verification runs it, so a body godwit could not plan is refused at generation rather than at apply. What that body is allowed to contain is settled in the amendment below.
...
## Amendment — the body is rendered for an empty database

#85 shipped the collapse and left the rendering to pg-schema-diff's defaults. The load rig in #86 then measured the feature on the shape it is for and found it did not pay: at 1000 migrations `plan` went from 31.9 s to 36.3 s and `diff` from 79.2 s to 81.1 s after collapsing. The collapse is not what was wrong — the thousand migrations really are recorded in one statement instead of replayed. What replaced them was a body of **3001 statements, 1000 of them `CREATE INDEX CONCURRENTLY`**, because the renderer builds every primary key as `CREATE TABLE`, then a concurrent unique index, then the constraint adopting it. That is the right shape for a live table and the wrong one here: the fourth row of the table above is the whole point — **a checkpoint is never run on a database that already holds part of what it carries**, so it is never run on a database with rows, readers or writers either.

So the body is now rendered for that context. Indexes carry no `CONCURRENTLY`, which puts every statement back inside a transaction; the index/constraint pair is folded back into the `CREATE TABLE`, as is the `NOT VALID`/`VALIDATE CONSTRAINT` pair every foreign key renders as; and a checkpoint's statements raise no hazards, because every hazard godwit knows is a statement about a populated, live table.

The same 1000 migrations now render as 1001 statements and 147 KB instead of 3001 and 275 KB, and the scratch replay is 1.9× faster on that purely additive shape and 76× faster on a churning one. What did *not* move is `godwit plan` end to end at that directory size, and measuring the replay on its own is what showed why: the replay is an eighth of it, and the rest is the request carrying 2001 files and the store persisting them. A checkpoint makes the replay cheap; it does not make a 2000-file request cheap, and nothing about the checkpoint should be expected to.

*Why not change the executor to run the whole body in one transaction instead.* That would be the larger saving and it is not the checkpoint's to make: the per-statement transaction is what the journal is, and a crash-safe resume depends on it. Rendering fewer statements gets most of it without touching the property the rest of godwit is built on.

*Why fold rather than post-process into `pg_dump`-style output.* The fold is an AST rewrite of one pattern the renderer is known to emit, and the fingerprint verification already applies the result and refuses a mismatch — so a fold that is wrong is a refusal on the author's machine, not a bad file. Anything the pattern does not match exactly is left alone.

*Why the generator pins `search_path` to `public`.* #86 also found `godwit checkpoint` renders an empty body and refuses on the setup `demo/docker-compose.yml` and `docs/getting-started.md` produce. The renderer excludes schema `godwit`; the replay's own `bootstrap` creates that schema; and nothing pinned the session's `search_path`, so on a scratch role also called `godwit` the default `"$user", public` resolved `$user` to the excluded schema and every unqualified `CREATE TABLE` landed in it. `Validator.Validate` was immune only because it mirrors the target's *observed* `search_path` (#49) and generation has no target to observe. Pinning is the fix that does not depend on what the role is called. **Rejected: reading the resolved path off the scratch database instead**, which is one more round trip for a value that must not vary between runs of the same directory.
