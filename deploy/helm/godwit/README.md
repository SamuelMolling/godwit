...
- A second PostgreSQL for the scratch databases validation and `godwit diff` execute submitted SQL on, with a role that owns nothing else. Skipping it leaves that execution on the store server as the store role, and the pods say so on every start ([security](../../../docs/run/security.md#the-scratch-database)).
...
Register them against the API instead, with an `admin` token that belongs to whatever registers ([deployment](../../../docs/run/deployment.md#registering-a-target)):
...
`serve.limits` sets the pool size, the request and file sizes, the migration file count, the concurrent runs and the concurrent scratch-database calls, and the per-run wall clock. The defaults hold roughly an order of magnitude more than a real migration directory; [deployment](../../../docs/run/deployment.md#admission-limits) says which one to raise for which refusal, and `serve.limits.maxConcurrentDiffs` is the one to size the scratch server’s `max_connections` and disk against.
