...
**The decision: one more statement, at the end of the expand phase.** `SELECT count(*) FROM <t> WHERE <c>_new IS DISTINCT FROM <expr>` with `= 0`, so `change-type` is 7 expand statements and 6 contract ones. The predicate is the string the batched `UPDATE` already selects on, hoisted rather than written a second time: the batches select it, the trigger's assignment makes it false for every row it is handed, and the assertion counts it — the same "one predicate, three consumers" that makes #89 sound.

**The position is the whole value.** Anywhere in the expand phase it would catch a bad backfill; last is what makes `ConfirmRollout` re-ask it. #82 has the executor re-evaluate an assertion it walks past, and the confirm walks the expand phase before the contract statements — so the count is asked once when the expand phase ends and again, against the data as it is then, immediately before the rename. *Rejected: a second copy spliced into the contract phase.* The re-check already gives that, and a copy would be a fourth expression to keep in step.

**Once the contract phase has begun, the assertion is past asking.** That re-check needed a bound it did not have: the swap renames `<c>_new` away, so a run that died between two contract statements would resume onto a query the schema no longer answers — a crash-resume regression in the one thing godwit promises. `Executor.apply` now re-evaluates a completed assertion only while `prog.lastDone` is still before the plan's first contract statement. Plans with one phase — every hand-written assertion, and `backfill`'s own — are unaffected, because there is no contract statement to be past.
