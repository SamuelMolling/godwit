...
2. Authorises the commander — `allowed-associations`, then the real permission lookup and the approval anchor described [below](#who-may-command-an-apply). Every refusal exits 1, says which check failed, and [answers on the pull request](#a-refusal-answers-on-the-pull-request).
