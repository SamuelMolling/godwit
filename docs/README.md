...
| [Command reference](cli.md) | You want to know what a command is *for* — every `godwit` command in plain language, grouped by what you are doing, with a real example each. |
...
Suggested order for a first read: getting started → command reference → concepts → configuration. Operators add deployment, operations and the runbook; pipeline authors add CI/CD.
