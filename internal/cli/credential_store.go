...
func newCredentialStoreAddCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "Every vault target names a store, and the store is the only thing that says which Vault its\n" +
			"credentials are read from. With --vault-k8s-role the service logs in at that Vault with a\n" +
			"ServiceAccount token minted for --vault-audience, so that Vault needs a Kubernetes auth mount\n" +
			"trusting this cluster, a role bound to it requiring that audience, and a deployment projecting\n" +
			"a token for it; --vault-token-env names an environment variable holding a token instead.",
	...
	cmd.Flags().StringVar(&req.VaultAudience, "vault-audience", "",
		"audience this Vault's Kubernetes auth role requires; the deployment projects a ServiceAccount "+
			"token for it, and that token is worthless at any other Vault")
...
func credentialStoresTable(stores []*godwitv1.CredentialStore) string {
	...
	for _, s := range stores {
		...
		if s.VaultK8SRole != "" {
			auth = "kubernetes " + s.VaultK8SMount + " as " + s.VaultK8SRole + " for " + s.VaultAudience
		}
