...
`Validate` creates a scratch database, replays the history migration by migration through the same `Executor` a target uses, then applies the pending plans and snapshots the schema after each one. Those milliseconds are round trips, and **eleven of them were `bootstrap`** re-creating tables that already existed: `Executor.apply` ensured the journal schema on every plan, and the replay builds one `Executor` per migration over one connection.

`engine.Session` now carries that fact on the connection, so bootstrap runs once per database. Counted at the `DB` interface, a replayed migration costs **19 round trips instead of 30** (30 and 19 include the six the bounded advisory lock and the session setup take since the chaos findings landed): three migrations through one session cost 30, 19, 19, and three through an `Executor` each still cost 30, 30, 30.
...
| Two in five of the scratch replay's round trips are `bootstrap` re-creating tables that exist, once per migration. | `internal/engine/executor.go` | fixed: `engine.Session` carries the bootstrap on the connection, so it runs once per database and a migration costs 19 round trips instead of 30 — 31% off the replay, a couple of seconds of a forty-second `plan`. `bootstrap` also stopped depending on `IF NOT EXISTS` being race-free |
