...
`deploy/helm/godwit/values.yaml` documents every value inline; the `serve` block exposes `port`, `driftInterval`, `skipValidation`, `scratch.template`, `logFormat`, `logLevel`, `ui.enabled`, `ui.scope` and `extraArgs`. **The Secret named by `existingSecret.name` reaches the container whole**: every entry in it becomes an environment variable of that name, so each row of the table above is configured in the Secret and in no value — adding `GODWIT_SCRATCH_DSN` is what moves scratch off the store server, adding `GODWIT_MASTER_KEY` is what lets a `static` target be registered, and neither is named twice. The one exception is the App's private key, which is mounted as a file rather than exported: `existingSecret.githubPrivateKey` names the entry holding the PEM, and the render fails if that name is one a shell would accept as a variable. `serve.keyProvider` moves the key to `gcpkms` or `vault-transit` instead. `--lease-ttl`, `--tick-interval`, `--max-attempts`, `--require-plan`, `--plan-ttl` and `--plan-retention` have no value of their own and go through `serve.extraArgs`. Standing the chart up on ArgoCD, and where the credentials in it come from, is [deployment](deployment.md).
