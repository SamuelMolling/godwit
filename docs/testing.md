...
| A directory of 1000 migrations is the ceiling on the default `--max-files 2000`: every request carries the whole directory at two files per migration, and a checkpoint adds a file rather than removing any — the collapsed migrations stay until every target is above the checkpoint and someone deletes them by hand. | `internal/api/limits.go` | fixed: admission counts migrations (`--max-migrations` 2000) and the file cap became a bound on everything else (`--max-files` 5000); `TestCheckFiles` pins a 1000-migration directory and its checkpoint as admissible |
