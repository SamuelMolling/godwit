...
(Narrowed in #157, once the webhook faced the internet. Two of the refusals below are reached before godwit knows anything about whoever commented, and this record did not weigh what that hands them: a check run is last-writer-wins on its `(name, head_sha)`, so anyone who can comment on a pull request could replace the green `godwit/plan` a reviewer is looking at with a red one and, where that check is required, hold the merge. A refusal now sets a check only once the delivery is past authorization; before that the comment is the whole answer. And an **unbound repository** is answered nowhere at all — no comment, no check, no installation token — because godwit serves that repository nothing and everything it would spend there is chosen by whoever commented. The rest of this record stands: the reaction, the standing refusal comment, and the check on every refusal the commander was entitled to ask for.)
