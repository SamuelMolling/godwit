...
| `/ui/targets/{name}/migrations/{migration}` | what one migration did on that target: every attempt in `godwit.runs`, each statement in order with its outcome and timing, the verifier behind a non-transactional one, and the error beside the statement that produced it |
...
| `/ui/plans` | every stored plan newest first, filtered by target (`?target=`) and state (`?state=ready\|bound\|superseded`), with what each state is waiting on — a `ready` plan names the apply it waits for, a `bound` one its run, a `superseded` one the plan that replaced it. The key prefix has the whole key behind it, and the provenance links the commit the files came from; a plan records the repository and commit, never the pull request |
| `/ui/plans/{id}` | one plan in full: its whole key and the commit it came from, statements per migration grouped by phase, every hazard with its recipe, `already applied by hand` with the effect it recorded, the directives a migration carried and the expansion they produced, the observation the plan was taken against, and the drift the target had at that moment |
| `/ui/drift` | which targets have drifted, as a list built from `ListTargets` — drifted first, `?scope=all` for every target, `?q=` to filter by name, `?page=` through 50 at a time, each view stating how many targets the fleet has. `?target=` is one target's events, with check and accept-baseline |
...
The rail and every target list come from `ListTargets`, so a registered target that was never migrated appears from the moment it is registered. The rail itself lists at most twelve, the ones with runs waiting for a human first, and links the rest to `/ui/targets`. The plan list asks `ListPlans` once per target. A plan that retention has swept renders as *pruned* rather than a `404`: the run keeps the record of what it applied.
...
`/ui/targets/{name}/migrations/{migration}` reads `GetMigrationJournal`. `godwit.journal` records a hash per statement, never its SQL, so the text on that page is the file bodies of the run that applied the migration, matched to the journal by hash; the [retention](#retention) sweep of `cp_run_files` leaves the statements as hashes, and the sweep of `godwit.journal` itself leaves the migration as *recorded without executing*.
