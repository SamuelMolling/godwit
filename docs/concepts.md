...
3. **Hazard gate** — every hazard code in the migrations this admission would *execute* must be in `acknowledge_hazards`; otherwise `failed_precondition` listing them. The whole directory is submitted every time, so the gate reads only what is pending: a migration the target already holds contributes none, on the same predicate that makes it `applied` in the plan, and neither does a checkpoint that is only being recorded. A down plan is the exception — a revert runs it *because* the target holds it, so `DROP TABLE` in a down file still needs `--ack H002`.
