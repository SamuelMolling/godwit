...
func reportItem(version int64, name string, statements ...engine.Statement) item {
	return item{Plan: engine.Plan{
...
func succeededReport() Run {
	...
	return Run{
		...
		plan:    Plan{live: true, target: "app", rollout: "direct", validated: true, items: []item{users}},
...
func heldReport() Run {
	...
	return Run{
		...
		plan: Plan{
			live: true, target: "app", rollout: "expand-contract", validated: true, items: []item{people},
		},
	}
}

func failedReport() Run {
	...
	r := Run{
		...
		plan: Plan{
			live: true, target: "app", rollout: "direct", validated: true, items: []item{first, second},
		},
		public: "https://godwit.example.com",
	}
...
func TestRunReportLeavesOutWhatTheRunDidNotCarry(t *testing.T) {
	...
	old.skipped = true
	r.plan.items = append([]item{old}, r.plan.items...)
	var b strings.Builder
	writeRunText(&b, r)
