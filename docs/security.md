...
**What a target's registration gives back.** `GetTarget` (`godwit target show`, `operator`) returns everything in that config column except the credential: the Vault path, the mounted file, the template, and for `static` only `dsn_registered: true`. The sealed DSN is never returned and never opened to be redacted — a read path that decrypts is a read path that can leak — and nothing needs it back, because `RegisterTarget` keeps it while another setting changes. The template is returned in full and is therefore held to being a template: one carrying a literal password instead of `{{password}}` is refused at registration, so that column cannot become a place a secret hides. `operator` rather than `read` is the same line drawn once more: `ListTargets` says how a target behaves, `GetTarget` says where its secret lives, and a Vault address (`read`, above) plus a path is a map.
