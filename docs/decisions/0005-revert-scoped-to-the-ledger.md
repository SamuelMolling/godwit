...
Shipped in #68. Completed by #71, which moved the scratch-validation replay onto the same ledger, and by #75, which took the run's own state back out of the predicate.
...
- **#75 then had to take `cp_runs.state` out of that predicate.** #68 and #71 both wrote it as `r.state = 'succeeded' AND a.reverted_by IS NULL`, which asks the *run's* state a question about *a migration's* fate. A run that applies three migrations and fails on the fourth leaves the three standing — the target's `godwit.migrations` records them, its journal records them, `cp_run_applied` records them — while the applied set, the applied count and the replay all behaved as if the run had done nothing. In practice: `godwit targets` reported 0 applied against a `godwit target status` that listed three; the out-of-order guard stopped guarding, because the newest applied version it compares against was gone; and, worst, a **directive** the failed run had applied was expanded a second time, against a scratch catalog rebuilt without it, so the next run would have tried to add a shadow column the target already had. The predicate is now `NOT a.held AND a.reverted_by IS NULL` — a property of the migration only. `held` is what the run state was standing in for and does it properly: it marks a migration whose contract phase never ran, so the target holds its expand statements but records nothing, and it becomes applied when the contract phase lands. The revert side keeps its own, wider scope (`reverted_by IS NULL`, held rows included), because a held migration is still work the run did and still has an inverse.
...
- **The replay never rebuilds a held migration, and that is deliberate.** #71 left the branch out as untestable, reasoning that a held row can only sit on an `awaiting_contract` run, which the query never returned. It is reachable — an `awaiting_contract` run whose *contract* phase then fails is `failed` with its held row intact — and the answer is still to leave it out: the target records nothing for that migration, so replaying half of it would put the scratch base in a state no migration id describes, and marking it *replayed* would make it the no-op #71 was fixing. The visible cost is that a plan taken over such a target reports the expand-phase objects as drift, which is the honest reading of a target parked mid-migration; the way out is `ResumeRun`, not a fresh plan. Now that the exclusion is `NOT a.held` in the shared predicate rather than a side effect of the run state, it is a stated rule with a test on it.
- **Retention got sharper teeth.** A settled run is not automatically disposable: a `failed` run can own the only copy of the bodies the replay needs. `docs/operations.md` now filters on standing `cp_run_applied` rows rather than on `cp_runs.state`.
